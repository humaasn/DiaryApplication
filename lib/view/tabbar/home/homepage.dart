...
class _HomePageState extends State<HomePage> {
               ...
               Container(
                ...
                Padding(
                  ...
                  child: ElevatedButton(
                    ...
                    check = await db.checkContent(Message.email, formattedDate);
                    if (check == 0){
                      final diary = Diary (email: Message.email, formattedDate: formattedDate, diarycontent: Message.content, workcontent: Message.workcontent, breakfast: Message.breakfast, lunch: Message.lunch, dinner: Message.dinner, waterAmount: Message.waterAmount, health: Message.health);
                      db.infoInsert(diary);
                       _completeshowDialog(context);
                    }else{
  ...
  void showDatePickerPop() {
   ...
   _completeshowDialog(BuildContext context){
    showDialog(
      context: context, 
      builder: (BuildContext context){
          return AlertDialog(
            title: const Text('다이어리 등록',
              style: TextStyle(
                fontFamily: 'GowunDodum',
              ),
            ),
            content: const Text('오늘의 추억이 저장되었습니다.',
              style: TextStyle(fontFamily: 'GowunDodum',),
            ),
            actions:[
              TextButton(
